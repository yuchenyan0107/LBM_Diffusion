...
function ic_2Dgauss_pulse(x, y, z, t)
  ...
  return (background + amplitude * math.exp(-0.5/(halfwidth^2)*r)) * cs_phy^2
  --return (amplitude*math.exp(-0.5/(halfwidth^2)*r)) * cs_phy * cs_phy
end
...
function ic_3Dgauss_pulse(x, y, z, t)
  ...
  --return (amplitude*math.exp(-0.5/(halfwidth^2)*r)) * cs_phy * cs_phy
  return (background + amplitude*math.exp(-0.5/(halfwidth^2)*r)) * cs_phy * cs_phy
end
-------------------------------------------------------------------------------
...
tracking = {
  ...
  --{
  --  label = 'vtk',
  --  variable = {'pressure_phy', 'absorblayer_box',
