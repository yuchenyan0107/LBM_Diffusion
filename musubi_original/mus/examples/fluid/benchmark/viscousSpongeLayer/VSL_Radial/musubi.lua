...
nu_phy = vel_mean_phy * dia_cyl / Re
...
press_ambient = rho0_phy * cs_phy * cs_phy
...
-- Circular sponge
sponge_start = height/2.0 - 10*dia_cyl
sponge_length = 10*dia_cyl

nu_lat = nu_phy * dt / dx^2
omega = 1.0 / ( nu_lat/cs_lat^2.0 + 0.5 )
damp_factor = 1.5
thickness = 10*dia_cyl
...
variable = {
  ...
  },
  {
    ...
    operation = {
      ...
      reduction_transient = {
        kind = 'average',
        nrecord = math.ceil(t_ramp/dt)-10
      }
    }
  },
  {
...
--  },
...

  {
    ...
    time_control = {min = 3*t_ramp-dt, max = tmax_phy, interval = t_ramp},
    --reduction = {'sum', 'sum'},
    -- ascii format writes variable values and simulation time into a single
    -- file at every tracking interval. It is usally used for point tracking.
