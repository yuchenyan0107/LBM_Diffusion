...
!  Used to call exit to provide a proper exit code.
...
!  Interface to C stdlib routine to return with an errorcode
  interface
    subroutine errorexit(i) bind (c, name='exit')
      use, intrinsic :: iso_c_binding
...
! Return an error code instead of exiting without one
!stop 'coco error exit'
write(*,*) 'coco error exit'
call errorexit(1)
