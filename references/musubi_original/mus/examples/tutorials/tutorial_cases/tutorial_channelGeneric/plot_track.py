...
#  os.remove(dbname)
  ...
  # Load tracking output with velAlongHeight and store in tabname=vel_spatial
  sqlcon = gleaner.tracking_to_db(fname = ['tracking/*shearAlongHeight*.res'], \
                                  dbname=dbname, tabname='shear_spatial')
  ...
  # Load tracking output with wssAlongHeight and store in tabname=wss_spatial
  sqlcon = gleaner.tracking_to_db(fname = ['tracking/*pressL2*.res'], \
                                  dbname=dbname, tabname='press_l2')

## -------------------------------------------------------------------------- ##
logging.info('Velocity profile across the channel height:')
fig = mplt.figure()
ax = fig.add_subplot(111)
# Extract coordinate-Y, velocity magnitude (from simulation)
# and analytical velocity
get_data_for_cols = ['coordY','velocity_phy_01', 'velRef']
...
mplt.plot(x, y, '-', color = 'k', label = 'Simulation')
mplt.plot(x, z, 's', color = 'b', label = 'Analytic')
...
mplt.savefig('VelocityX_Profile.png', dpi=100, format='png', \
...
get_data_for_cols = ['coordX','pressure_phy', 'pressureRef']
...
# Plot simulation result
mplt.plot(x, y, '-', color = 'k', label = 'Simulation')
mplt.plot(x, z, 's', color = 'b', markevery=4, label = 'Analytic')
...
mplt.savefig('Pressure_Profile.png', dpi=100, format='png', \
             bbox_inches="tight",interpolation=None)
## -------------------------------------------------------------------------- ##
logging.info('Shear stress magnitude profile across the channel height:')
fig = mplt.figure()
ax = fig.add_subplot(111)
# Plot x, y ... at certain timestep
# Extract coordinate-Y, wall shear stress (from simulation)
# and analytical wall shear stress
get_data_for_cols = ['coordY', 'shear_mag_phy']
[x, y] = gleaner.get_columns(sqlcon, tabname='shear_spatial', \
                             columns=get_data_for_cols)
# sort loaded data according to x
...
fig = mplt.gcf()
fig.set_size_inches(figsize[0],figsize[1])
mplt.savefig('ShearMag_Profile.png', dpi=100, format='png', \
             bbox_inches="tight",interpolation=None)
## -------------------------------------------------------------------------- ##
...
[x, y] = gleaner.get_columns(sqlcon, tabname='wss_spatial', \
                             columns=get_data_for_cols)
# sort loaded data according to x
x, y = zip(*sorted(zip(x,y))) # sort of needed
# Plot simulation result
...
get_data_for_cols = ['time','press_fluc']
...
# plot setting
mplt.xlabel('time (s)')
mplt.ylabel("Pressure fluctuation p' ($Pa$)")
mplt.grid(True,which="major",ls="-")
ax.yaxis.set_major_formatter(y_formatter)
mplt.xlim(0.0,10.0)
